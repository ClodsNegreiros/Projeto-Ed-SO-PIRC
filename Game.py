...
class Game:
    ...
    def getQntdTentativas(self):
        return self.__qntTentativas
    ...
    def verificarLetra(self, letra: str):
        ...
        if not letra in self.__palavra:
            ...
            self.__qntTentativas -= 1 # Diminui = a quantidade de tentativas disponíveis
    ...
    def __getForcaLetras(self) -> str:
        ...
        for i in range(len(self.__palavra)):
            if self.__palavra[i] in self.__letrasAchadas.lista():
                ...
                s += f'_, '
        s += f'\nVocê possui {self.__qntTentativas} tentativas restantes\n'
        return s
